    ...
    def generate_predict_patch(image, patch_size, batch_size, mean):
        ...
        for r_indx in range(0, image.shape[0], patch_size):
            for c_indx in range(0, image.shape[1], patch_size):
                x_patch = np.ubyte(image[r_indx:r_indx+patch_size,c_indx:c_indx+patch_size,:])
                c1, c2, c3 = cv.split(x_patch)
                x_patch = cv.merge((cv.equalizeHist(c1), cv.equalizeHist(c2), cv.equalizeHist(c3)))

...
def iou_my(true, pred):
    # btrue = K.cast(true, bool)
    # btrue = K.cast(btrue, float)
    # bpred = K.cast(pred, bool)
    # bpred = K.cast(bpred, float)
    btrue = K.greater(true, 100)
    btrue = K.cast(btrue, float)
    bpred = K.greater(pred, 100)
    bpred = K.cast(bpred, float)
    intersection = btrue * bpred
    notTrue = 1 - btrue
    union = btrue + (notTrue * bpred)
    return (2 - (1 + K.sum(intersection, axis=-1)) / (1 + K.sum(union, axis=-1)))* 10000

def iou_with_mse(true, pred):
    btrue = K.cast(true, bool)
    btrue = K.cast(btrue, float)
    bpred = K.cast(pred, bool)
    bpred = K.cast(bpred, float)
    ...
    mse = K.mean(K.square(pred - true), axis=-1)
    return (1 + K.epsilon() - iou) * K.exp(mse/2000) /1000000

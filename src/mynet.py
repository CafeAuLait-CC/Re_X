...
def build_rmse(img_size,nclasses=6):
    ps0,ps1,depth = img_size
    ...
    ifinal_input = Concatenate()([ia1,input])
    out = _conv2D(1, 1, padding="valid")(ifinal_input)
...
def build(img_size,nclasses=6):
    ...
    a1 = _conv2D(16,5,dilation=2,padding="same")(input)
    
    m1 = MaxPooling2D((2,2))(a1)
    
    b1 = _conv2D(32,3,dilation=3,padding="same")(m1)
    b2 = _blocker(1,32,cardinality=16)(b1)
    
    midl = _conv2D(64,3,padding="valid")(b2) 
    
    ib1 = _conv2DTran(32,3,padding="valid")(midl)
    
    ib1_b1 = Concatenate()([ib1,b1])
    ib1_ = _blocker(1,32,cardinality=16)(ib1_b1)
    
    im1 = UpSampling2D((2,2))(ib1_)
    
    im1_a1 = Concatenate()([im1,a1])
    ia1 = _conv2DTran(16,3)(im1_a1)

    iinput_a1 = Concatenate()([ia1,input])

    ifinal = _conv2D(nclasses, 1, padding="valid")(iinput_a1)
    i = Reshape((ps0*ps1,nclasses))(ifinal)
    iax = Activation("softmax")(i)
    out = Reshape((ps0,ps1,nclasses))(iax)
    
    #mod = Model(inputs=input, outputs=out)
    #print(mod.summary())
    return Model(inputs=input, outputs=out)
    
def build_aug(img_size,nclasses=6):
    ps0,ps1,depth = img_size

    input = Input((ps0,ps1,depth))

    a1 = _blocker(1,32,cardinality=4)(input)
    a2 = MaxPooling2D((2,2))(a1)

    b1 = _blocker(1,64,cardinality=8)(a2)
    b2 = MaxPooling2D((2,2))(b1)
    
    c1 = _blocker(1,128,cardinality=16)(b2)
    c2 = MaxPooling2D((2,2))(c1)
    ...
    ic2 = UpSampling2D((2,2))(imid)
    ic2_c1 = Concatenate()([ic2,c1])
    ic1 = _blocker(1,64,cardinality=8)(ic2_c1)

    ib2 = UpSampling2D((2,2))(ic1)
    ib2_b1 = Concatenate()([ib2,b1])
    ib1 = _blocker(1,32,cardinality=4)(ib2_b1)

    ia2 = UpSampling2D((2,2))(ib1)
    ia2_a1 = Concatenate()([ia2,a1])
    ia1 = _blocker(1,16,cardinality=2)(ia2_a1)
